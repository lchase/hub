...
const authCheck = jwt({
  secret: new Buffer('hub_auth_secret', 'base64'),
  audience: 'hub_auth_client_id'
});
...
app.get('/api/contacts/:id', authCheck, (req, res) => {
  res.josn(contacts.filter((contact) => contact.id === parseInt(req.params.id)));
});

app.listen(4001);
console.log('API listening on http://localhost:4001');